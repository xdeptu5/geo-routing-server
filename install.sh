...
install_wizard() {
    ...
    echo ""
    echo -e "  ${BOLD}2)${NC} Сервер раздачи баз и правил (универсальный Hub для Remnawave, Marzban, Incy, Happ)"
    echo -e "     ${DIM}Раздает базы geoip/geosite, диплинки Happ и JSON для Incy с вашего домена.${NC}"
    echo -e "     ${DIM}Панели и клиенты забирают файлы по ссылкам, API-токен панели серверу не нужен.${NC}"
