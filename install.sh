...
install_wizard() {
    ...
    echo ""
    echo -e "  ${BOLD}4)${NC} Фоновый сервис синхронизации Remnawave (домен на этой машине не нужен)"
    echo -e "     ${DIM}Базы уже на другом сервере (или GitHub). Контейнер работает как локальный воркер${NC}"
    echo -e "     ${DIM}в Docker и передает правила напрямую в API сквадов Remnawave.${NC}"
    echo ""
