...
class HappProcessor(BaseProcessor):
    """Обработчик файлов для клиента HAPP (geo-базы, модификация JSON, DEEPLINK и автоочистка)."""
    ...
    def _discover_config_files(self) -> List[str]:
        ...
        api_url = f"{Config.get_github_api_base()}/HAPP"
        try:
            ...
            logger.warning(f"GitHub API discovery failed for HAPP ({e}), using fallback file list")
            ...
            if name_lower.endswith(".json") or name_lower.endswith(".deeplink"):
                if path.name not in valid_filenames:
                    try:
                        ...
                        logger.info(f"  Removed obsolete HAPP file: {path.name}")
                    except Exception as e:
    ...
    def process(self) -> bool:
        ...
        # 2. Синхронизируем geoip.dat и geosite.dat с их sha256
        if not self.geo_manager.sync_client_geo(client, target_dir, default_json_data):
            success = False
            
        # 3. Синхронизируем и модифицируем JSON конфигурации для HAPP
        ...
        for file_name in config_files:
            ...
            try:
                ...
                # Сохраняем стабильный LastUpdated (для HTTP 304)
                ...
                # Генерируем DEEPLINK (схема happ://routing/onadd/<base64>)
