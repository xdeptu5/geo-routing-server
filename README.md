...
|---|---|---|
| **Главное меню** | `geoserver` | Нумерованное меню управления сервером |
| **Статус и ссылки** | `geoserver status` или пункт `1` | Состояние контейнера; пункт меню также показывает готовые ссылки |
| **Синхронизация** | `geoserver sync` или пункт `2` | Принудительное обновление баз и правил |
| **Изменение настроек** | Пункт `3` | Отдельные разделы: клиенты, сеть, расписание, интеграции и источники |
| **Конфиги реверс-прокси** | Пункт `4` | Генератор конфигов Caddy, Nginx и NPM с учётом расположения прокси |
| **Обновление** | `geoserver update`, `update-image`, `update-script` или пункт `5` | Обновление всех компонентов либо выбранного компонента |
| **Логи** | `geoserver logs` или пункт `6` | Просмотр логов контейнера в реальном времени |
| **Контейнер** | `geoserver start`, `restart`, `stop` или пункт `7` | Запуск, перезапуск и остановка |
| **Дополнительно** | Пункт `8` | Язык интерфейса, системная информация и удаление |
| **Выход** | Пункт `9` | Выход из меню |
