...
Самый простой способ. Скрипт проверит наличие Docker (или установит его), спросит каталог установки (для Portainer / стеков), домен, предложит ввести свой токен или сгенерирует надёжный автоматически:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/xdeptu5/geo-routing-server/main/install.sh)
```

> **Совет:** После установки вы можете в любой момент вызвать удобное меню управления сервером командой:
> ```bash
> geo-server
> ```
...
### Способ 2. Запуск готового образа через Docker Compose (Вручную)
...
| `ENABLED_CLIENTS` | `HAPP,INCY` | Список активных клиентов через запятую (`HAPP`, `INCY` или `HAPP,INCY`) |
