...
> **Управление:** После установки доступно меню управления командой `geoserver` (или короткой `grs`). Повторный запуск скрипта открывает то же меню и никогда не перезаписывает данные без подтверждения.
...
|---|---|---|
| **Главное меню** | `geoserver` *(или `grs`)* | Интерактивное управление, смена параметров, логи |
...
Скрипт определит существующую установку и откроет меню <code>geoserver</code> (или <code>grs</code>). Конфигурация и данные никогда не перезаписываются без явного подтверждения.
</details>
