...
### Сценарий 4. Фоновый сервис синхронизации Remnawave (базы на внешнем узле)
Работает как внутренний воркер на сервере с Remnawave. **Свой публичный домен, реверс-прокси и открытые порты не требуются.** Контейнер загружает правила, подставляет адреса внешнего гео-узла (Сценарий 3), кодирует диплинки и передаёт их напрямую в API сквадов Remnawave:
```env
